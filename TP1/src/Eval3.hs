...
update v n (mp, trace) = ((M.insert v n mp), trace)

-- Agrega una traza dada al estado
addTrace :: String -> State -> State
addTrace str (mp, trace) = (mp, trace ++ str) 
...
stepComm (Let v e)  s  = case (evalIntexp e s) of
                         (Right (n :!: s')) -> Right (Skip :!: (addTrace ("Let "++v++" "++(show n)++"\n") (update v n s')))
                         (Left  err) -> (Left  err)
...
evalIntexp (EAssgn var e) s = case (evalIntexp e s) of
                              Right (n :!: s') -> Right (n :!: (addTrace ("Let "++var++" "++(show n)++"\n") (update var n s')))
                              (Left err) -> Left err
